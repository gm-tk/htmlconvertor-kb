...
> **When to load:** Whenever a message contains the trigger phrase **`UPDATE MODE`** (case-insensitive) — typically accompanied by, or immediately followed by, the changes the designer wants permanently implemented (a finalized Comparison Mode difference report, or free-typed instructions in any format). This is **Mode 4 — Update** (see `00_MASTER_INSTRUCTIONS.md` → Operating Modes). Together with `COMPARISON MODE`, the `UPDATE MODE` trigger takes precedence over the ordinary Conversion / Advisory / Support mode signals. **`ADMIN MODE` outranks it**: a message carrying both is an **Admin Mode** run (`17_ADMIN_MODE.md`, constraint 86) — Update Mode is for a **designer's** proposed change and keeps its scope questions, its conflict pause, its lock block and its escalation path; Admin Mode is for an **authorised** change that is already approved.
...
The ledger serves four jobs, and is organised into four parts so the conflict-relevant data stays compact as the history grows:
...
- **Log the front-facing ones to Part 4 as well.** For every actioned change, apply the front-facing test (constraint 87, `17_ADMIN_MODE.md` → Section 5.1): *would a developer comparing two generated modules — one built before the change, one after, from the same Writers Template — see any difference in the HTML or CSS?* **Yes, or borderline → draft an entry for the PageForge Amalgamation Log** (`12_CHANGE_LEDGER/12G_PAGEFORGE_AMALGAMATION_LOG.md`) into the same Repo Update Brief, alongside the ordinary ledger row. **No → the ledger row only.** This applies to Update Mode exactly as it applies to Admin Mode; the log is the project's front-facing decision record, not an Admin Mode artefact.
...
| **Locked (admin)** | A change actioned through **Admin Mode** by an authorised individual (`17_ADMIN_MODE.md`). Binding on Update Mode exactly like `Locked` — a conflicting Update Mode change is `Blocked (conflict)` — but released **without** an unlock: another `ADMIN MODE` message supersedes it. | Part 1 (+ a history row in Part 3) |
...
3. **If a prior entry targets the same thing with a DIFFERENT result** → it is a **conflict**. Resolve it by the prior entry's lock state and by the **intake channel** (Section 4.3) of *both* sides:
   - **Prior entry is `Locked (admin)`** → **BLOCK.** Do not action the new change. Draft a `Blocked (conflict)` ledger row and tell the designer plainly: *this contradicts a decision made through Admin Mode by an authorised individual ([ID], [date]); changing it needs a new `ADMIN MODE` instruction from Gavin or the lead designer — an unlock from the design authority will not release it.* Never route it to Persephone.
...
**Admin Mode is outside this section entirely.** A change actioned through `ADMIN MODE` is never catalogued, never provisional and never awaits sign-off — it overrides whatever it contradicts, including a `Locked` row, and is recorded `Locked (admin)` (`17_ADMIN_MODE.md` → Section 4, constraint 86). No Admin Mode run ever produces the block in Section 7.1.

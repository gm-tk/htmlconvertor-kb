...
| The message contains `PAGEFORGE COMPARE MODE` / `PAGEFORGE COMPARISON MODE` / `PAGEFORGE COMPARE` + uploaded PageForge-generated HTML + the `{CODE}_interactives.txt` worklist (a required input — ask for it if absent). **Never load or mention this topic on any other trigger** | `16_PAGEFORGE_COMPARE_MODE.md` (`16A` mode core + inputs + upload formats, `16B` what to report and what to ignore, `16C` the report format) + `01_PIPELINE_EXTRACTION_TAGS.md` (sections 04–05, tag taxonomy + interpretation — the writer tags are the measure) + `03`/`04`/`05` COMP files for any component in a finding + `15_INTERACTIVES_BUILD_MODE.md` (`15B`) when reading a `{CODE}_interactives.txt` worklist as evidence |

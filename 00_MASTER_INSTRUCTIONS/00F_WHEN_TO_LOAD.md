...
> **Granular part F (6 of 8) of `00_MASTER_INSTRUCTIONS.md`** — When to load which files.
...
| The message contains `COMPARISON MODE` + uploaded finished HTML (and **not** the leading word `PAGEFORGE`) | `09_COMPARISON_MODE.md` (whole file) — **unless** the uploaded page is an assessment page (`alertAssessment` / `template="NCEA"`) in a chat that ran Assessment Mode: then `18_ASSESSMENT_MODE/18C_ASSESSMENT_COMPARISON_MODE.md` + `09B` (exclusions) + `09C` Section 9 (the finalized bundle) |
| The message contains `PAGEFORGE COMPARE MODE` / `PAGEFORGE COMPARISON MODE` / `PAGEFORGE COMPARE` + uploaded PageForge-generated HTML (PageForge's ORIGINAL, un-stitched output — the only upload; never ask for a worklist). **Never load or mention this topic on any other trigger** | `16_PAGEFORGE_COMPARE_MODE.md` (`16A` mode core + inputs + upload formats, `16B` what to report and what to ignore, `16C` the report format) + `01_PIPELINE_EXTRACTION_TAGS.md` (sections 04–05, tag taxonomy + interpretation — the writer tags are the measure) + `03`/`04`/`05` COMP files for any component in a finding + `15_INTERACTIVES_BUILD_MODE.md` (`15B`) ONLY if a tester volunteers a `{CODE}_interactives.txt` and it must be read as corroborating evidence |
| The message contains `ADMIN MODE` (an authorised change follows in any format) — **check this trigger before every other one; it outranks them all** | `17_ADMIN_MODE.md` (whole file) + `12_CHANGE_LEDGER.md` (Part 1 to record what is overridden; `12G` when the change is front-facing) + `11_UPDATE_MODE.md` (Sections 9, 10, 13 — the blast-radius sweep, the Repo Update Brief format, the timestamp convention, all reused unchanged) |
| Deciding whether a change must also be recorded in the PageForge Amalgamation Log | `17_ADMIN_MODE.md` (Section 5 — the front-facing test and the two lanes) + `12_CHANGE_LEDGER/12G_PAGEFORGE_AMALGAMATION_LOG.md` |
| The message contains `ASSESSMENT MODE` (any capitalisation), **or** an uploaded `.docx` is an NCEA Assessment Activity document (details table `Activity name:` … `Brief description:` + the bold sections What to do / How to present your learning / Timeframe / Getting started / Student Resources / Dropbox) — with or without any instruction | `18_ASSESSMENT_MODE.md` (`18A` trigger + mapping + skeleton, `18B` content rules + acknowledgements + To Do list) + `05_COMP_LANGUAGE_MEDIA_LAYOUT.md` (`05C` — acknowledgement entry formats) + `02_DATA_CONTENT_VERIFICATION.md` (Comment & Red Flag Policy — every gap is a visible `Designer/Developer To Do:`) |

...
> **Granular part A (1 of 12) of `12_CHANGE_LEDGER.md`** — Ledger purpose, status values, PART 1 locked decisions (CL-0086 to CL-0094), PART 2 pending approval. (PART 3 is the history parts; **PART 4 is the PageForge Amalgamation Log** in `12G`.)
...
| CL-0093 | Sunday, 13th September, 2026 | **The language-font classes `jp-text`, `ch-text` and `pinyin` are MANDATORY on every occurrence — the conversion applies them, not the designer afterwards.** Japanese → `jp-text`, Chinese → `ch-text`, pinyin → `pinyin`, on every run in every context, the whole run wrapped including its internal punctuation. **Te reo Māori and English are never wrapped** — a macron alone never identifies pinyin. Where shared Han characters cannot separate Japanese from Chinese, the class comes from the module's language; an undeterminable case is red-flagged, never guessed. | (c) Universal | `05A` Language Fonts; `00H` constraint 92; `02C`; `00C` | **Locked (admin)** — authorised channel, 13 September 2026 (design-team instruction relayed to Gavin). Released only by a later `ADMIN MODE` message. |
| CL-0094 | Sunday, 13th September, 2026 | **The *Responding to Suspected Use in Assessments* AI Guidelines PDF was renamed in Central Files.** Tag `[Kaimahi AI Guidelines- Responding to Suspected Use in Assessments PDF]` → filename `Kaimahi AI Guidelines Responding to Suspected Use in Assessments.pdf` — year range dropped, no hyphen in the filename, `Use` capitalised. The retired tag wording is still matched so templates in flight keep working; the retired filename is never emitted. Amends constraint 84; its other seven assets are unchanged. | (c) Universal | `05A` AI Guidelines registry; `00G` constraint 84 (amended); `01D`; `01E`; `02C` | **Locked (admin)** — authorised channel, 13 September 2026 (Central Files naming change relayed to Gavin). Released only by a later `ADMIN MODE` message. |
